...
export class CheckoutService {
    ...
    async criarIngressos(input) {
        ...
        if (preco <= 0)
            throw new AppError(400, "INVALID_PRICE", "Preço do ingresso inválido.");
    ...
    async criarInscricao(input) {
        ...
        if (duplicate)
            throw new AppError(409, "ALREADY_ENROLLED", "Você já está inscrito nesta turma.");
        const folder = this.ensureQrFolder();
        const qrcodeId = randomUUID();
    ...
    async atualizarStatusIngresso(id, status) {
        return prisma.ingresso.update({ where: { id }, data: { status } });
    }
    async atualizarStatusInscricao(id, status) {
