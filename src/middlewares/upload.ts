...
export async function processUploadToCloudinary(req: Request, res: Response, next: NextFunction) {
  try {
    if (req.file) {
      const uploadedImage = await uploadToCloudinary(req.file);
      (req as Request & { uploadedImage?: unknown }).uploadedImage = uploadedImage;
    }
    ...
    if (req.files && Array.isArray(req.files)) {
      ...
      (req as Request & { uploadedImages?: unknown[] }).uploadedImages = uploadedImages;
    }
