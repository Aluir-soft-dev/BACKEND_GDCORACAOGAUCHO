...
function sanitizeTaxId(value?: string | null) {
  return (value ?? "").replace(/\D/g, "");
}

function sanitizeCellphone(value?: string | null) {
  return (value ?? "").replace(/\D/g, "");
}
...
export class AbacatePayService {
  async createEventoCheckout(input: {
    userId: string;
    ingressos: Array<{ id: number; preco: number; evento: { nomeEvento: string } }>;
  ...
  async createCursoCheckout(input: {
    userId: string;
    inscricao: { id: number; preco: number; turma: { nome: string } };
    user: { name: string; email: string; phone?: string | null; cpf?: string | null };
  }) {
    const { returnUrl, completionUrl } = getUrls();
  ...
  async processWebhook(payload: { event?: string; data?: { checkout?: { id?: string; status?: string } } }) {
    ...
    if (!record) {
      return { handled: false, reason: "PAYMENT_RECORD_NOT_FOUND" as const };
    }
    ...
    if (event === "checkout.completed") {
      ...
      return { handled: true, action: "PAID" as const, record };
    }
    ...
    return { handled: false, reason: "EVENT_IGNORED" as const, event };
  }
}
